<template>
  <div class="addEdit-block">
    ...
    >
      ...
      <el-col :span="12">
        <el-form-item class="select" v-if="type!='info'"  label="影院名称" prop="yingyuanmingcheng">
          <el-select v-model="ruleForm.yingyuanmingcheng" placeholder="请选择影院名称">
            <el-option
                v-for="(item,index) in yingyuanmingchengOptions"
                v-bind:key="index"
                :label="item"
                :value="item">
            </el-option>
...
export default {
  ...
  methods: {
      ...
      this.$http({
        ...
        if (data && data.code === 0) {
          ...
          this.$message.error(data.msg);
            ...
            this.$http({
              url: `option/yingyuanxinxi/yingyuanmingcheng`,
              method: "get"
            }).then(({ data }) => {
              if (data && data.code === 0) {
                this.yingyuanmingchengOptions = data.data;
              } else {
                this.$message.error(data.msg);
              }
            });
         
            this.fangyingtingOptions = "1号厅,2号厅,3号厅,4号厅,5号厅,6号厅,7号厅,8号厅".split(',')
    },
    // 多级联动参数
    info(id) {
      this.$http({
	...
	this.ruleForm.dianyingjianjie = this.ruleForm.dianyingjianjie.replace(reg,'../../../ssm443x6/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
